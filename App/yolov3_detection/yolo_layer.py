...
class YoloLayer(nn.Module):
    ...
    def forward(self, output, target=None):
        if self.training:
            ...
            t0 = time.time()
            # print('yolo_layers_122', type(output))
            nB = output.data.size(0)
            nA = self.num_anchors
            nC = self.num_classes
            nH = output.data.size(2)
            nW = output.data.size(3)
            # print('output of model', output.shape)
            ...
            coord_mask = Variable(coord_mask.cuda())# mask
            ...
            self.loss_conf = nn.MSELoss(size_average=False)(conf*conf_mask, tconf*conf_mask)/2.0
            loss_cls = self.class_scale * nn.CrossEntropyLoss(size_average=False)(cls, tcls)
            loss = loss_x + loss_y + loss_w + loss_h + self.loss_conf + loss_cls
            t4 = time.time()
            """
            if False:
                print('-----------------------------------')
            ...
            masked_anchors = []
            # print('output of forward---------------------------', output.shape)
