...
class RegionLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        nA = self.num_anchors
        nC = self.num_classes
        nH = output.data.size(2)#final size of output, simultaneously can be viewed as [h, w] of grid cell
        ...
        coord_mask = Variable(coord_mask.cuda())#create mask for calculation of loss of bbox coordinates, shape such as [bx,by,bw,bh,0,0.....]
        ...
        loss_cls = self.class_scale * nn.CrossEntropyLoss(size_average=False)(cls, tcls)#classification using CrossEntropyLoss
        loss = loss_x + loss_y + loss_w + loss_h + loss_conf + loss_cls#sum up total loss above as final region loss
        t4 = time.time()
        """
        if False:
            print('-----------------------------------')
